...
<div class="container" style="max-width:500px;">
    <div class="card p-4 shadow-sm">
        <h4 style="color:#a0785a; font-weight:700;" class="mb-4">✏️ Edit Artis</h4>
        <form action="/artis/{{ $artis->id }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label class="form-label fw-bold">Nama</label>
                <input type="text" name="nama" class="form-control" value="{{ $artis->nama }}" required>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Genre</label>
                <input type="text" name="genre" class="form-control" value="{{ $artis->genre }}" required>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Negara</label>
                <input type="text" name="negara" class="form-control" value="{{ $artis->negara }}" required>
            </div>
